    ...
    // ======= [新增] 喵喵分析資料（由你的 A/B 結果整理） =======
      window.MEOW_ANALYTICS = {
        ...
        city_brand_counts: {
          ...
          // ======= [新增] 喵喵縣市分析外掛（智慧辨識：縣市別名 + 行政區推縣市） =======
        (function(){
          const form = document.getElementById("botForm");
          const input = document.getElementById("botText");
          if(!form || !input) return;

          // 你已經有 window.MEOW_ANALYTICS（在外掛上面）
          ...
          form.addEventListener("submit", function(e){
            const text = input.value || "";
            const city = detectCitySmart(text);

            if(!city) return; // 沒命中 → 讓原本 botReply 流程處理
            ...
            if(typeof window.addMessage === "function"){
              ...
              window.addMessage(buildCityReport(city), "bot");
              input.value = "";
            }
          }, true);
        })();
